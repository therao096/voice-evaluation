...
#p="F:/acciobis/pro1/myprosody-master/myprosody"
#m="output"

def some_function(a):
    b = '"' + a + '"'
    return b
#m=some_function(filename)
# In[7]:
    ...
    try:
        ...
        for i in range(25):
            ...
            if he==0:
                he=25
                dfout = "%s:\t %f (%s)" 
                #dfout = "%s:\t %f (%s)" %  (nsns[i]"% percentile, between 0 & 25% ")
                aa=dfout+nsns[i]+"  :between 0 & 25%" + " ,score:"+ str(score)
                results.append(aa)
                #print(dfout,",score:",score)
            elif he>=25 and he<=75:
                dfout = "%s:\t %f (%s)" 
                bb=dfout+nsns[i]+"  :between 25% & 75%" +" ,score:"+ str(score)
                results.append(bb)
                #print(dfout,",score:",score)
            else:
                dfout = "%s:\t (%s)" % (nsns[i],he)
                cc=dfout+"score:"+ str(score)+" out of range"
                results.append(cc)
        return results        #print(dfout,"%percentile",",score:",score,"out of range")
